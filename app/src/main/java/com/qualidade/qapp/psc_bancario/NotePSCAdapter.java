...
import com.qualidade.qapp.rejeitos_dod.NoteDodAdapter;

import java.util.ArrayList;
import java.util.List;

public class NotePSCAdapter extends RecyclerView.Adapter<NotePSCAdapter.MViewHolder> {
    private List<NotePSC> notePSC = new ArrayList<>();
    ...
    public void onBindViewHolder(@NonNull MViewHolder holder, int position) {
        NotePSC currentNote = notePSC.get(position);
        ...
        if (notePSC.get(position).getStatus().equals("OK")) {
            holder.textViewStatus.setTextColor(Color.parseColor("#93C47D"));
        }
        else if (notePSC.get(position).getStatus().equals("NOK")) {
    ...
    public void setNotePSC(List<NotePSC> notePSC) {
        this.notePSC = notePSC;
